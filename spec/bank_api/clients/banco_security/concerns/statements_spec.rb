...
RSpec.describe BankApi::Clients::BancoSecurity::Statements do
  ...
  describe "current_statement" do
    ...
    describe "#select_current_statement" do
      before do
        allow(browser).to receive(:search)
          .with(".cuentas-corrientes").and_return(double(any?: false))
        allow(div).to receive(:attribute).with("data-numero-cuenta").and_return(account_number)
      end

      it "doesn't raise error" do
        expect { dummy.select_current_statement }.not_to raise_error
      end
      ...
      context "with wrong account" do
        ...
        it "raises error" do
          expect { dummy.select_current_statement }.to raise_error(
      ...
      context "with multiple accounts" do
        ...
        before do
          allow(browser).to receive(:search)
            .with(".cuentas-corrientes").and_return(double(any?: true))
          rows.zip(links).each do |row, link|
            allow(row).to receive(:search).with("td").and_return([link])
          end
          allow(link[1]).to receive(:click)
        end
